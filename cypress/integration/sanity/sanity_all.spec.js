...
const states = [
    {
        "suite_name": "(Peru) Administrator Role Sanity Tests",
...
states.forEach((state) => {
    describe(state.suite_name, () => {
        before(() => {
            cy.visit('/')
            cy.login(state.user, state.project)
        })
        ...
        it('CCSDK_TC_SAN_04 Check that a journal entry can be added from the journal manager', () => {
            ...
            cy.getFirstLine('#journal-table')
                .should(($divs) => {
                    ...
                    expect($divs.eq(4)).to.deep.contain(Cypress.dayjs().format('MM/DD/YYYY'))
                }).wait(2000)
                .eq(5).find('button[data-testid=delete-entry]').click({force: true})
        ...
        it('CCSDK_TC_SAN_07 Check that an admin can add a cross section', () => {
            ...
            cy.getLastLine('#geo-table')
                .should(($divs) => {
                    ...
                    expect($divs.eq(3)).to.deep.contain(Cypress.dayjs().format('MM/DD/YYYY'))
                })
        })

        it('CCSDK_TC_SAN_08 Check that an admin user can add a pond level sensor', () => {
            cy.visit('/module-loader/Data/GeoJsonEditor', {failOnStatusCode: false})
            cy.get('header h6').should('contain', 'Cross Sections')
            ...
            cy.get('ul[role="listbox"]')
                .find('li').contains("Emulated - PZ-10").click()
            cy.get('#profileId').should('contain',"Emulated - PZ-10")
            cy.get('[role="dialog"]')
        ...
        it('CCSDK_TC_SAN_10 Check file upload and deleting', () => {
            ...
            cy.getFirstLine('#file-table')
                .should(($divs) => {
                    ...
                    expect($divs.eq(5)).to.deep.contain(Cypress.dayjs().format('MM/DD/YYYY'))
                })
